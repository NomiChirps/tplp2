// Configuration file for LVGL v8.3.1
#ifndef LV_CONF_H
#define LV_CONF_H
 ...
 *internal processing mechanisms. You will see an error log message if there
 *wasn't enough buffers. */
...
#define LV_TICK_CUSTOM_INCLUDE \
  "tplp/lv_tick_custom.h" /*Header for the system time function*/
#define LV_TICK_CUSTOM_SYS_TIME_EXPR                                        \
  (lv_tick_custom_impl()) /*Expression evaluating to current system time in \
                             ms*/
...
/**
 * "Simple layers" are used when a widget has `style_opa < 255` to buffer the
 * widget into a layer and blend it as an image with the given opacity. Note
 * that `bg_opa`, `text_opa` etc don't require buffering into layer) The widget
 * can be buffered in smaller chunks to avoid using large buffers.
 *
 * - LV_LAYER_SIMPLE_BUF_SIZE: [bytes] the optimal target buffer size. LVGL will
 * try to allocate it
 * - LV_LAYER_SIMPLE_FALLBACK_BUF_SIZE: [bytes]  used if
 * `LV_LAYER_SIMPLE_BUF_SIZE` couldn't be allocated.
 *
 * Both buffer sizes are in bytes.
 * "Transformed layers" (where transform_angle/zoom properties are used) use
 * larger buffers and can't be drawn in chunks. So these settings affects only
 * widgets with opacity.
 */
 ...
 *cache in bytes. If the cache is too small the map will be allocated only while
 *it's required for the drawing. 0 mean no caching.*/
#define LV_GRAD_CACHE_DEF_SIZE 0

/*Allow dithering the gradients (to achieve visual smooth color gradients on
 *limited color depth display) LV_DITHER_GRADIENT implies allocating one or two
 *more lines of the object's rendering surface The increase in memory
 *consumption is (32 bits * object width) plus 24 bits * object width if using
 *error diffusion */
#define LV_DITHER_GRADIENT 0
#if LV_DITHER_GRADIENT
/*Add support for error diffusion dithering.
 *Error diffusion dithering gets a much better visual result, but implies more
 *CPU consumption and memory when drawing. The increase in memory consumption is
 *(24 bits * object's width)*/
...
// TODO: disable logging for release
#define LV_USE_LOG 1
#if LV_USE_LOG
...
// tplp: We register it in InitLvgl().
...
// Do not use any demos or examples.
