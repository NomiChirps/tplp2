#ifndef PICOLOG_STATUSOR_INTERNAL_H_
#define PICOLOG_STATUSOR_INTERNAL_H_
...
void test(char (*)[sizeof(std::declval<U>().operator util::StatusOr<T>())]);
...
struct IsDirectInitializationAmbiguous<T, util::StatusOr<V>>
...
template <typename T, typename U>
struct IsForwardingAssignmentAmbiguous<T, util::StatusOr<U>>
...
class Helper {
 public:
  ...
  [[noreturn]] static void Crash(const util::Status& status);
};
...
[[gnu::nonnull(1)]] void PlacementNew(void* p, Args&&... args) {
...
class StatusOrData {
 ...
 public:
  ...
  template <typename U,
            std::enable_if_t<std::is_constructible<util::Status, U&&>::value,
                              int> = 0>
  ...
  void AssignStatus(U&& v) {
    Clear();
    status_ = static_cast<util::Status>(std::forward<U>(v));
    EnsureNotOk();
  }
...
#endif  // PICOLOG_STATUSOR_INTERNAL_H_